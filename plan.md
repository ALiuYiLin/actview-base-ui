...
### ✅ 已完成（37 个）
`combobox`（16 测试通过 ✅：Root/Input/Trigger/InputGroup/Portal/Positioner/Popup/List/Item/ItemIndicator/Group/GroupLabel/Value/Label/Icon/Clear/Chips/Chip/ChipRemove/Arrow/Backdrop/Collection/Separator/Row/Empty/Status 全组件 + items/createItems + itemCollection + useFilter/useFilteredItems + store class；修复 AI-003 复发（ComboboxGroup `return wrappedElement` 变量→`<>{wrappedElement}</>`）、setup 内 `if (!shouldRender) return null` 提前返回固定（Portal/Clear/ItemIndicator 改 JSX 条件渲染）、props getter 展开覆盖 on* handler（ComboboxInput `...inputProps.value` 顶掉 listNavigation onKeyDown→mergeProps 链）、computed 内调 context hook（useInjects 只能在 setup：ComboboxList 的 useComboboxPositionerContext 移出 computed、useBaseUiId 移出 computed）、watch 数组源误放函数被当 getter 调用（setIndices 从依赖数组移除）、floatingRootContext 元素需手动同步（useListNavigation floatingElement 为 null 导致 456 分支不执行→watch 同步 reference/floating）、onInput 替代 onChange（actview 监听原生 input 事件）、items prop 时 CompositeList 会以 stale DOM 覆盖 listRef（跳过 CompositeList，Item 走 indexFromFilter 响应式分支）、Chip 注册加 guess、highlighted useStore a1 传 ref 而非快照、aria-hidden 查询用 `[aria-hidden]`（PD-01）、fireEvent.input 需 inputType、render 用 `render(Component, props)` 而非 JSX）、`autocomplete`（13 测试通过 ✅：Root/Value/Trigger/Item/InputGroup/Separator 独有文件 + Input/Icon/Clear/Popup/Positioner/List/Row/Arrow/Backdrop/Portal/Group/GroupLabel/Empty/Status/Collection 复用 Combobox 别名；react 版 Autocomplete.Root 即 AriaCombobox 薄包装：selectionMode='none' + fillInputOnItemPress + mode→autoComplete 映射 + inline 临时值；本轮修复 5 个深层通用 bug：① AriaCombobox filterQuery 改代理读（mode=both 的 typed query 过滤，原解构快照恒为初始值）② context computed 值变化不驱动消费者重渲染（ComboboxInput 显示滞后）→ store 加 inputValue 镜像字段 + 同步 watch + 订阅（QA: combobox-context-render-reactivity）③ ComboboxInput `const renderedInput = getElement()` 在 setup 缓存 VNode（useRenderElement getter 必须在渲染函数调用，AD-38）→ 内联 return JSX ④ ComboboxItem listRef 注册 watch 依赖加响应式 itemElement（immediate 在 setup 跑时 ref 未挂载，首渲染 index 不变不重注册 → clickHighlightedItem 失效）⑤ useListNavigation aria-activedescendant reference/floating 分流（typeable combobox 的 input 侧应返回 `${id}-${activeIndex}`，QA: floating-activedescendant-split）+ DirectionContext useInjects 移出 computed（useInjects 只能在 setup））、`accordion`（11 测试通过 ✅：root/item/header/trigger/panel 全组件 + contexts 补全；依赖 collapsible 子系统；修复 AD-27 getter 链合并 prev 保 id/children、aria-expanded 布尔归一化）、`avatar`、`button`（7 测试通过 ✅）、`checkbox`、`checkbox-group`、`collapsible`、`csp-provider`、`direction-provider`、`field`（12 测试通过 ✅：Error/Label/Item/Validity/Control 补全 + index 导出；修复 Form getter 链丢失 onSubmit（AD-20 变体）、useLabel htmlFor→for（AD-24）、FieldError children 覆盖 + messageIds 替换式注册（AD-25））、`fieldset`、`form`（依赖 Form.tsx 修复 getElementProps 合并）、`input`（7 测试通过 ✅，依赖 FieldControl 已修）、`internals`、`menu`（16 测试通过 ✅：Root/Trigger/Portal/Positioner/Popup/Item/LinkItem/CheckboxItem+Indicator/RadioGroup/RadioItem+Indicator/Group/GroupLabel/SubmenuRoot/SubmenuTrigger/Arrow/Backdrop/Viewport 全组件 + MenuStore/MenuHandle + 前置 menubar/MenubarContext + context-menu/root/ContextMenuRootContext；修复 AI-003 复发（MenuRoot `return content` 变量保持裸函数→`<>{content}</>`）、props 链 getter 展开覆盖 on* handler（submenu trigger useClick onMouseDown 被 `{...prev,...rest}` 顶掉；全组件统一 mergeProps 合并——AD-20/AD-27 全面审计）、watch 卸载期 stale 微任务 undefined 解构补 Array.isArray（AD-33 漏网 3 处：useTransitionStatus/useOpenChangeComplete/useSyncedFloatingRootContext）、aria-checked/aria-hidden 布尔属性字符串化（PD-01）、Enter 键盘激活测试补手动 click（AD-19））、`merge-props`、`meter`、`number-field`（13 测试通过 ✅：root/group/input/increment/decrement/scrub-area/scrub-area-cursor 全组件补全；修复 AD-20 getter 整体替换丢属性、AD-21 隐藏 input setup 静态对象、AD-22 Teleport 替代 createPortal、AD-23 测试组件顶层定义，见 plantform-diff.md）、`popover`（19 测试通过 ✅：Root/Trigger/Portal/Positioner/Popup/Arrow/Backdrop/Title/Description/Close/Viewport + Store/Handle 全组件补齐；依赖 popups/floating-ui-actview 基建已就绪；修复 AD-28~AD-31 + resolveRef/FFM children/watch 数组源等，见 plantform-diff.md）、`progress`、`radio`、`radio-group`、`scroll-area`（13 测试通过 ✅：root/viewport/scrollbar，jsdom 适配见 plantform-diff.md AD-16）、`select`（17 测试通过 ✅：Root/Trigger/Value/Icon/Label/Portal/Positioner/Popup/List/Item/ItemText/ItemIndicator/Arrow/Backdrop/Group/GroupLabel/Separator/ScrollUpArrow/ScrollDownArrow 全组件 + SelectStore class；修复 useTransitionStatus watch 数组源防御（AD-33 延伸）、portal 条件渲染不能 setup return null、props 响应式必须 getter 求值，见 plantform-diff.md）、`separator`、`slider`（23 测试通过 ✅：root 10 + utils 13；修复 AI-003：组件末尾 `return <>{getElement()}</>` 规避 Babel 转换漏检，见 actview-issue.md）、`switch`、`tabs`（6 测试通过 ✅：AI-001 解决链完成——AI-002 框架修复 + props 静态对象改 getter（AD-17）+ 测试 waitFor flush（AD-18）+ jsdom 键盘模拟（AD-19），见 actview-issue.md AI-001）、`toast`（33 测试通过 ✅：Provider/Viewport/Root/Content/Action/Close/Title/Description/Portal/Positioner/Arrow 全组件 + ToastStore class + createToastManager/useToastManager；store 21 测试移植 + 组件 12 测试；修复 toast 特有：toast prop 必须 computed 响应式（PD-15）、渲染元素组件的 elementProps 含 children 快照会覆盖 getter（须 getter 从 componentProps 重读）、AI-003 return 包装、waitFor 只在抛错时重试，见 plantform-diff.md AD-36/AD-37）、`toggle`、`toggle-group`、`toolbar`、`types`、`unstable-use-media-query`、`use-render`、`utils`

### ⏳ 未开始（15 个）
| 组件 | 说明 |
|---|---|
| floating-ui-react | 口径误差：已完整移植为 `floating-ui-actview`（37 文件、类型检查干净） |
| alert-dialog、context-menu、dialog、drawer、menubar、navigation-menu、otp-field、preview-card、tooltip | 待逐个实现（otp-field 命中框架问题 AI-001/AI-002，与 tabs 一起待框架修复后实现） |
