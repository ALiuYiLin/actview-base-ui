...
const requestCallback: (fn: () => void) => number = supportsIdleCallback
  ? (fn) => requestIdleCallback(fn)
  : (fn) => setTimeout(fn, 0) as unknown as number;
const cancelCallback: (id: number) => void = supportsIdleCallback
  ? cancelIdleCallback
  : ((id: number) => clearTimeout(id)) as (id: number) => void;
