...
export function useScrollLock(
  ...
  referenceElement: MaybeRefOrGetter<Element | null> = null,
) {
  watch(
    () => [toValue(enabled), toValue(referenceElement)] as const,
    ([isEnabled, refEl], _old, onCleanup) => {
      if (!isEnabled) {
        return;
      }
      const release = SCROLL_LOCKER.acquire(refEl);
      onCleanup(release);
    },
    { immediate: true },
  );
