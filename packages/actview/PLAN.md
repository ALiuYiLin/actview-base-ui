...
### 批次 4：大族（最后，调用点最多）
- [~] combobox/autocomplete 进行中——AutocompleteRootContext/ComboboxRootContext hook 已 store-as-is 化；ComboboxRoot Provider 载体化 + selectedValue computed；**autocomplete 9/9 绿**。余：combobox 其余组件（Context 4 错已清）+ select（31）
