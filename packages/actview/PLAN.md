...
- **泄漏扫描（golden 之外的同类错误）**：按 C15 同类模式（自定义 props 未从 toRefs 剔除 → elementProps → DOM）全库扫描，发现并修复 **14 个组件**（`67f42344d`）：radio（RadioRoot/RadioGroup，`value` 实证泄漏到 span/group div）、switch（SwitchRoot `value/name/uncheckedValue/checked`）、form（Form `onSubmit` 会经 elementProps 覆盖 rootProps 的框架 submit 流程——validation+preventDefault 被绕过）、field（actionsRef）、number-field（inputRef）、tabs（TabsRoot/TabsTab/TabsPanel）、meter/progress（MeterRoot/ProgressRoot format/locale/max/min/value/getAriaValueText）、checkbox-group、navigation-menu（NavigationMenuRoot）、menu radio-group。验证方式：渲染探针 dump outerHTML 断言无框架属性 + 全量 1143/1143 绿。附带结论：**RadioRoot 既有 `[object Object]` 渲染错误已随 `76efc4da5` 的 ref 剔除修复**（探针实证 standalone 与 group 组合均干净）。
