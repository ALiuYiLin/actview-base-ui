...
export function NumberFieldRoot(componentProps: NumberFieldRoot.Props) {
  ...
  const disabled = computed(() => fieldRootContext.value.disabled || (componentProps.disabled ?? false));
  ...
  const value = useControlled<number | null>({
  ...
  const valueRef: RefObject<number | null> = { current: null };
  watch(
    () => value.value,
    (v) => {
      valueRef.current = v ?? null;
    },
    { immediate: true },
  );
  ...
  watch(
    () => value.value,
    (v) => {
      fieldRootContext.value.setFilled(v !== null);
    },
    { immediate: true },
  );
  ...
  const inputValue = ref(
    formatNumber(value.value ?? null, componentProps.locale, componentProps.format),
  );
  const inputMode = ref<InputMode>('numeric');

  const allowInputSyncRef: RefObject<boolean> = { current: true };
  ...
  // Local tick replacing the deleted `useForcedRerendering`: `setValue` bumps it so formatting
  // changes still re-render even when the numeric value did not change. It is read inside
  // `getHiddenInputProps`, which is evaluated in the render function, establishing the dependency.
  const tick = ref(0);
  ...
  function getAllowedNonNumericKeys() {
    ...
    if (minWithDefault < 0 || (componentProps.allowOutOfRange ?? false)) {
  ...
  function getStepAmount(event?: EventWithOptionalKeyState) {
    if (event?.altKey) {
      return componentProps.smallStep ?? 0.1;
    }
    if (event?.shiftKey) {
      return componentProps.largeStep ?? 10;
    }
    const stepProp = componentProps.step ?? 1;
    return stepProp === 'any' ? 1 : stepProp;
  }
  ...
  function setValue(
    ...
    const shouldClampValue = !(componentProps.allowOutOfRange ?? false) || !isInputReason;

    const validatedValue = toValidatedNumber(
      unvalidatedValue,
      dir ? getStepAmount(eventWithOptionalKeyState) * dir : undefined,
      componentProps.min ?? Number.MIN_SAFE_INTEGER,
      componentProps.max ?? Number.MAX_SAFE_INTEGER,
      componentProps.min ?? 0,
      componentProps.format,
      componentProps.snapOnStep ?? false,
    ...
    if (shouldFireChange) {
      ...
      value.setValueIfUncontrolled(validatedValue);
      fieldRootContext.value.setDirty(
        validatedValue !== fieldRootContext.value.validityData.initialValue,
      );
    ...
    if (allowInputSyncRef.current) {
      inputValue.value = formatNumber(validatedValue, componentProps.locale, componentProps.format);
    }
  ...
  // each event handler.
  watch(
    () => [value.value, componentProps.locale, componentProps.format],
    () => {
      ...
      const nextInputValue = formatNumber(
        value.value ?? null,
  ...
  watch(
    () => [disabled.value, readOnly.value, componentProps.allowWheelScrub ?? false],
    (_nv, _ov, onCleanup) => {
      const element = inputRef.current;
      if (
        disabled.value ||
        readOnly.value ||
        !(componentProps.allowWheelScrub ?? false) ||
        !element
      ) {
        return;
      }
      ...
      onCleanup(addEventListener(element, 'wheel', handleWheel));
    },
    { immediate: true },
  );
  ...
  const contextValue = computed<NumberFieldRootContext>(() => ({
    inputRef,
    minWithDefault: componentProps.min ?? Number.MIN_SAFE_INTEGER,
    maxWithDefault: componentProps.max ?? Number.MAX_SAFE_INTEGER,
  ...
  function getHiddenInputProps() {
    ...
    return fieldRootContext.value.validation.getValidationProps(disabledValue, {
      ...
      onInput(event: Event) {
        ...
        formContext.value.clearErrors(nameValue);
        fieldRootContext.value.validation.change(lastChangedValueRef.current ?? parsedValue);
      },
    });
  }
