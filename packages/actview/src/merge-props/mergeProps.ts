...
type PropsOf<T> = WithBaseUIEvent<
  (T extends keyof JSX.IntrinsicElements ? JSX.IntrinsicElements[T] : JSXPropsOf<T>) & {
    ref?: any;
  }
>;
 ...
 * The rightmost handler can prevent prior (left-positioned) handlers from executing
...
function copyInitialProps<T extends ElementType>(inputProps: any) {
...
/**
 * ActView dispatches native DOM events, which have no synthetic wrapper. Any object
 * passed as the first argument of a handler is treated as preventable, mirroring the
 * `preventBaseUIHandler` semantics of the React version.
 */
function isPreventableEvent(event: unknown): event is Event {
