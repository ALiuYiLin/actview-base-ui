...
export function useAnimationsFinished(
  elementOrRef: HTMLElement | null | { current?: HTMLElement | null; value?: HTMLElement | null },
  ...
  return useStableCallback(
    (
      ...
      const done = () => {
        // ActView has no `flushSync` equivalent; the callback is invoked directly.
        fnToExecute();
      };

      if (
        typeof resolvedElement.getAnimations !== 'function' ||
        (globalThis as Record<string, unknown>).BASE_UI_ANIMATIONS_DISABLED
      ) {
        fnToExecute();
        return;
      }
