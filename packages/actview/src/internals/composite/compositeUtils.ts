/**
 * Local, framework-agnostic inlining of the helpers that the React composite module imported
 * from `floating-ui-react/utils` and `floating-ui-react/utils/composite` (both unported).
 * Logic is preserved line-for-line; only the framework types were adapted:
 * - `React.KeyboardEvent` → native `KeyboardEvent`
 * - `React.RefObject<...>` → `{ current: ... }`
 * - `floor` from `@floating-ui/utils` → `Math.floor`
 */
