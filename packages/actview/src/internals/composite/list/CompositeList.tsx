...
export function CompositeList<Metadata>(props: CompositeList.Props<Metadata>) {
  ...
  const itemsRef = { current: null as readonly CompositeListItem<Metadata>[] | null };
  const mutationObserverRef = { current: null as MutationObserver | null };
  ...
  // the originating event is still inside the synchronous batch.
  ...
  const syncRefs = (items: readonly CompositeListItem<Metadata>[]) => {
  ...
  useIsoLayoutEffect(() => {
    return () => {
      mutationObserverRef.current?.disconnect();
      // Mark the retained map dirty so a later remount rebuilds refs and observation.
      isDirtyRef.current = true;
    };
  });
