...
export function ScrollAreaViewport(componentProps: ScrollAreaViewport.Props) {
  ...
  const lastMeasuredViewportMetricsRef = {
    current: [NaN, NaN, NaN, NaN] as [number, number, number, number],
  };
  ...
  function computeThumbPosition() {
    ...
    const scrollbarYEl = root.value.scrollbarYRef.current;
    const scrollbarXEl = root.value.scrollbarXRef.current;
    const thumbYEl = root.value.thumbYRef.current;
    const thumbXEl = root.value.thumbXRef.current;
    const cornerEl = root.value.cornerRef.current;
    ...
    if (isFirstMeasurement) {
      root.value.setHasMeasuredScrollbar(true);
    }
    ...
    if (!scrollbarXHidden) {
      ...
      scrollLeftFromStart = normalizeScrollOffset(
        direction.value === 'rtl' ? -scrollLeft : scrollLeft,
        maxScrollLeft,
      );
    ...
    const cornerNotYetSized = root.value.cornerSize.width === 0 && root.value.cornerSize.height === 0;
    ...
    root.value.setThumbSize({ width: clampedNextWidth, height: clampedNextHeight });
    ...
    if (scrollbarXEl && thumbXEl) {
      ...
      const scrollFromStart = direction.value === 'rtl' ? -scrollLeft : scrollLeft;
      ...
      thumbXEl.style.transform = `translate3d(${direction.value === 'rtl' ? -offsetX : offsetX}px,0,0)`;
    }
    ...
    if (cornerEl) {
      ...
      root.value.setCornerSize({ width: nextCornerWidth, height: nextCornerHeight });
    }

    root.value.setHiddenState(nextHiddenState);

    const nextOverflowEdges = {
      xStart: !scrollbarXHidden && scrollLeftFromStart > root.value.overflowEdgeThreshold.xStart,
      xEnd: !scrollbarXHidden && scrollLeftFromEnd > root.value.overflowEdgeThreshold.xEnd,
      yStart: !scrollbarYHidden && scrollTopFromStart > root.value.overflowEdgeThreshold.yStart,
      yEnd: !scrollbarYHidden && scrollTopFromEnd > root.value.overflowEdgeThreshold.yEnd,
    };

    root.value.setOverflowEdges(nextOverflowEdges);
  }
  ...
  // Watch scalar computed refs (not object literals from the root context) with a
  // `post` flush: ActView compares watch sources by reference, so object sources
  // recreated on every context recompute would retrigger this watch forever
  // (see plantform-diff.md AD-13).
  const hiddenY = computed(() => root.value.hiddenState.y);
  const hiddenX = computed(() => root.value.hiddenState.x);
  const hiddenCorner = computed(() => root.value.hiddenState.corner);
  const thresholdXStart = computed(() => root.value.overflowEdgeThreshold.xStart);
  const thresholdXEnd = computed(() => root.value.overflowEdgeThreshold.xEnd);
  const thresholdYStart = computed(() => root.value.overflowEdgeThreshold.yStart);
  const thresholdYEnd = computed(() => root.value.overflowEdgeThreshold.yEnd);
  ...
  useIsoLayoutEffect(() => {
    ...
    // being hovered.
    if (root.value.viewportRef.current?.matches(':hover')) {
      root.value.setHovering(true);
    }
  });

  useIsoLayoutEffect(() => {
    const viewport = root.value.viewportRef.current;
    ...
    waitForAnimationsTimeout.start(0, () => {
      // `getAnimations` is unavailable in jsdom; guard so SSR/jsdom tests don't throw.
  ...
  const viewportState = computed(() => root.value.viewportState);

  const getViewportProps = (prev: HTMLProps): HTMLProps => ({
    ...prev,
    role: 'presentation',
    ...(root.value.rootId && { 'data-id': `${root.value.rootId}-viewport` }),
    // https://accessibilityinsights.io/info-examples/web/scrollable-region-focusable/
    // Keep non-scrollable viewports out of tab order.
    tabIndex: root.value.hiddenState.x && root.value.hiddenState.y ? -1 : 0,
    ...
    onScroll() {
      ...
      // user-driven instead.
      if (root.value.touchModality || !programmaticScrollRef.current) {
        root.value.handleScroll({
  ...
  const getElement = useRenderElement('div', componentProps, {
    ref: [componentProps.ref, root.value.viewportRef],
    state: viewportState,
    props: [getViewportProps, getElementProps],
