...
describe('ToastStore', () => {
  ...
  it('prepends new toasts and limits the oldest active ones', () => {
    const store = createStore([]);

    // `applyLimited` flags active toasts at `activeIndex >= limit` (default limit 3),
    // so only the 4th (oldest) toast is limited.
    store.addToast({ id: 'a', title: 'A', timeout: 0 });
    store.addToast({ id: 'b', title: 'B', timeout: 0 });
    store.addToast({ id: 'c', title: 'C', timeout: 0 });
    store.addToast({ id: 'd', title: 'D', timeout: 0 });

    expect(selectors.toasts(store.state).map((toast) => toast.id)).toEqual(['d', 'c', 'b', 'a']);
  ...
  it('upserts an existing toast in place and refreshes its updateKey', () => {
    const store = createStore([]);

    store.addToast({ id: 'a', title: 'First', timeout: 0 });
    const updateKey = selectors.toast(store.state, 'a')?.updateKey;

    const returnedId = store.addToast({ id: 'a', title: 'Second', timeout: 0 });
    expect(returnedId).toBe('a');
    expect(store.state.toasts).toHaveLength(1);
    expect(selectors.toast(store.state, 'a')?.title).toBe('Second');
    expect(selectors.toast(store.state, 'a')?.updateKey).toBe((updateKey ?? 0) + 1);
  });

  it('closes all toasts when no id is provided', () => {
    const store = createStore([]);
    const onClose = vi.fn();

    store.addToast({ id: 'a', title: 'A', timeout: 0, onClose });
    store.addToast({ id: 'b', title: 'B', timeout: 0, onClose });
    store.closeToast();
  ...
  describe('auto-dismiss timeout', () => {
    ...
    it('closes a toast after its timeout elapses', () => {
      vi.useFakeTimers();
      const store = createStore([]);

      store.addToast({ id: 'a', title: 'A', timeout: 100 });
      vi.advanceTimersByTime(99);
      expect(selectors.toast(store.state, 'a')?.transitionStatus).toBe('starting');

      vi.advanceTimersByTime(1);
      expect(selectors.toast(store.state, 'a')?.transitionStatus).toBe('ending');
    });

    it('does not schedule a timer for loading toasts or a zero timeout', () => {
      vi.useFakeTimers();
      const store = createStore([]);

      store.addToast({ id: 'loading', title: 'Loading', type: 'loading', timeout: 100 });
      store.addToast({ id: 'zero', title: 'Zero', timeout: 0 });

      vi.advanceTimersByTime(1000);
      expect(selectors.toast(store.state, 'loading')?.transitionStatus).not.toBe('ending');
      expect(selectors.toast(store.state, 'zero')?.transitionStatus).not.toBe('ending');
    });

    it('reschedules the timer when the timeout prop changes', () => {
      vi.useFakeTimers();
      const store = createStore([]);
