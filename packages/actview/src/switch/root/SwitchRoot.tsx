...
export function SwitchRoot(componentProps: SwitchRoot.Props) {
  ...
  const getRoot = useRenderElement('span', componentProps, {
    ...
    props: [
      ...
      getElementProps,
      getButtonProps,
      (props) => fieldRootContext.value.validation.getValidationProps(disabled.value, props),
    ],
  ...
  return (
    <SwitchRootContext.Provider value={state}>
      {getRoot()}
      {!checked.value && name.value && componentProps.uncheckedValue !== undefined && (
        <input
          type="hidden"
          form={componentProps.form}
          name={name.value}
