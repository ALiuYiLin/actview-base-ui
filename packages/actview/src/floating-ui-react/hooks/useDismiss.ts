...
export interface UseDismissProps {
   ...
   * element. A lazy getter invoked when handling reference press events.
   * @default false
   */
 ...
 * (actview 版：store 模式；open/floatingElement 为 ComputedRef 读 .value。)
 */
export function useDismiss(
  ...
  const closeOnReferencePress = useStableCallback((event: any) => {
    if (!referencePress()) {
      return;
    }
  ...
  const closeOnEscapeKeyDown = useStableCallback((event: any) => {
  ...
  const markPressStartedInsideReactTree = useStableCallback((event: any) => {
  ...
  const markInsidePressStartPrevented = useStableCallback((event: any) => {
  ...
  watch(
    ...
    () => {
      ...
      function closeOnPressOutsideCapture(event: PointerEvent | MouseEvent) {
        ...
        // Only `pointerdown` marks a press; `mousedown` is its compatibility event.
  ...
  return enabled ? {reference, floating, trigger: reference} : {};
}
