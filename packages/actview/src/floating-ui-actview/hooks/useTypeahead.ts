...
export function useTypeahead(
  ...
  const prevIndexRef: { current: number | null } = {
    current: selectedIndexRef.current ?? activeIndexRef.current ?? -1,
  };
  const matchIndexRef: { current: number | null } = { current: null };
  ...
  function onKeyDown(event: KeyboardEvent) {
    ...
    if (isNewSession) {
      prevIndexRef.current = selectedIndexRef.current ?? activeIndexRef.current ?? -1;
    }
    ...
    const prevIndex = isNewSession
      ? (selectedIndexRef.current ?? activeIndexRef.current ?? -1)
  ...
  watch(
    [open, () => selectedIndexRef.current],
    ([openValue, selectedIndexValue]) => {
      if (!openValue && selectedIndexValue !== null) {
        return;
      }
