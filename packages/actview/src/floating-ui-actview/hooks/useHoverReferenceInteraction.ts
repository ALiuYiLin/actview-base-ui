import { isRef, onUnmounted, watch } from 'actview';
...
export function useHoverReferenceInteraction(
  ...
  watch([domReferenceElement], ([_domReferenceValue], _old, onCleanup) => {
    if (!enabled) {
      return;
    }
    ...
    function onMouseLeave(event: MouseEvent) {
      ...
      const domReferenceEl = store.select('domReferenceElement');
      const doc = ownerDocument(domReferenceEl);
      instance.restTimeout.clear();
      instance.restTimeoutPending = false;
      ...
      if (handleCloseRef.current && handleCloseContextBase) {
        ...
        const basePlacement = handleCloseContextBase.placement;
        const placementValue = isRef(basePlacement) ? basePlacement.value : basePlacement;
